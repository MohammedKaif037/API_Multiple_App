...
const generateMockCrossword = (theme: string, difficulty: number): CrosswordPuzzle => {
  // Simple working crossword that actually intersects properly
  const mockClues = {
    across: [
      { number: 1, clue: "Feline pet", answer: "CAT", startRow: 0, startCol: 0 },
      { number: 4, clue: "Rodent", answer: "RAT", startRow: 2, startCol: 0 }
    ],
    down: [
      { number: 1, clue: "Automobile", answer: "CAR", startRow: 0, startCol: 0 },
      { number: 2, clue: "Flying mammal", answer: "BAT", startRow: 0, startCol: 2 }
    ]
  }
...
function buildGridFromClues(clues: { across: CrosswordClue[], down: CrosswordClue[] }): string[][] {
  ...
  // Mark all cells that will contain letters
  ...
  for (let row = 0; row < GRID_SIZE; row++) {
    for (let col = 0; col < GRID_SIZE; col++) {
      ...
      if (letterCells.has(cellKey)) {
        ...
        grid[row][col] = "" // Empty/blocked cell
      }
    }
  }
...
export default function CrosswordGameClient() {
  ...
  const startGame = async () => {
    setLoading(true)
    try {
      // Using mock data - replace with your API call
